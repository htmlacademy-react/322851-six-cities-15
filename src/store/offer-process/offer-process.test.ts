...
describe('Offer-process slice', () => {
  ...
  it('Should set currentOffer to detailedOffers[3] uploadOfferById.fulfilled', () => {
    ...
    const result = offerProcess.reducer(state, uploadOfferById.fulfilled(currentOffer, '', '1'));
  ...
  it('Should set nearbyOffers to offers.slice(3) with uploadNearbyOffers.fulfilled', () => {
    ...
    const result = offerProcess.reducer(state, uploadNearbyOffers.fulfilled(newOffers, '', '1'));
  ...
  it('Should set rewviews to reviews.slice(2) with uploadReviews.fulfilled', () => {
    ...
    const result = offerProcess.reducer(state, uploadReviews.fulfilled(newReviews, '', '1'));
  ...
  it('Should add newReview with uploadNewReview.fulfilled', () => {
    ...
    const result = offerProcess.reducer(state, uploadNewReview.fulfilled(newReview, '', {offerId: '1', comment: '', rating: 5, disableForm: (status) => status}));

    expect(result.reviews?.find((review) => review.id === newReview.id)).toEqual(newReview);
  });
  ...
  it('Should not replace currentOffer with toggleFavoriteStatus.fulfilled', () => {
    ...
    const result = offerProcess.reducer(state, toggleFavoriteStatus.fulfilled(newOffer, '', {offerId: '1', status: 0}));
  ...
  it('Should replace newOffer in nearbyOffers with toggleFavoriteStatus.fulfilled', () => {
    ...
    expect(result.nearbyOffers?.find((offer) => offer.id === newOffer.id)).toEqual(newOffer);
  });
